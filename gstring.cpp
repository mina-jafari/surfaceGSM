...
void GString::setup_surface_sites()
{
    ...
    for (int i=0;i<nnmax;i++)
        surfs[i].setAtoms(natoms,coords[0],anames);

    printf("\n  Generating surface locations \n");
    surfs[0].findAllSites();

    string bfname = "scratch/bindSites.xyz"+nstr;
    printf("  writing to %s \n",bfname.c_str());
    surfs[0].writeToFile(bfname);

    int natoms1 = natoms;
    double* newX = new double[3];
    for (int i=0;i<nadd;i++)
    {
        if (add[2*i+0]>add[2*i+1])
        {
            int t1 = add[2*i+0];
            add[2*i+0] = add[2*i+1];
            add[2*i+1] = t1;
        ...
        int a2 = add[2*i+1];
        printf("   ADD: %2i-%2i \n",add[2*i+0]+1,add[2*i+1]);
        if (a2+1>natoms)
        {
            printf("   Uses surface binding site \n");
            printf("    natoms: %2i a2: %2i index: %2i \n",natoms,a2,a2-natoms+nbsites);
            update_coord_binding_site(coords[0],a2-natoms+nbsites,natoms);
            add[2*i+1] = natoms;
            natoms++; nbsites++;
        }
    }
    ...
    printf("  found %2i active binding sites \n",nbsites);
    //printf("  X %8.5f %8.5f %8.5f \n",newX[0],newX[1],newX[2]);
    //sites[0] = surfs[0].getBindingSite1(0);
    //sites[0].getXYZ(newX);

    for (int i=1;i<nnmax;i++)
        for (int j=3*natoms1;j<3*natoms;j++)
            coords[i][j] = coords[0][j];

    delete [] newX;

    printf("\n DONE with setup_surface_sites \n\n");
    return;
}
...
#include "savefile.cpp"
    ...
    if (tstype==-1)
    {
        printf(" TS_FINAL_TYPE is -1, ending now \n");
        exit(1);
    }
    ...
    grad1.init(infile0,natoms,anumbers,anames,runNum,runend,ncpu,1); 
    ...
    grad1.write_on = 0;
    if (initialOpt<1 || !isSSM)
        V0 = grad1.grads(coords[0], grads[0], icoords[0].Ut, 3);
    else V0 = V_profile[0];
    V_profile[0] = 0.;
    if (!isSSM)
    {
        grad1.init(infile0,natoms,anumbers,anames,runNum,runend+nnmax-1,ncpu,1); 
    ...
    osteps = 3;
    oesteps = osteps*2;
    ic_reparam_steps = 25; //CPMZ was 2!

    int resetHint = 1; //not using
...
int GString::isomer_init(string isofilename)
{
    ...
    //loop over additions, find new 4 coordinate species
    //which to choose?
#endif
...
void GString::init(string infilename, int run, int nprocs){
    ...
    if (tstype>0) 
    {
        printf("\n\n XYZ-SE-GSM only implemented for TS_FINAL_TYPE 0 \n");
        exit(1);
    }
...
void GString::structure_init(string xyzfile){
    ...
    if (!infile)
    {
        printf(" Error opening xyz file: %s \n",xyzfile.c_str());
        exit(-1);
    }
    ...
    int npad = 1;
    if (isSSM) npad += 20; //get extra space for X's

    anumbers = new int[npad+natoms];
    amasses = new double[npad+natoms];
    anames = new string[npad+natoms];
    frozen = new int[npad+natoms];
    for (int i=0;i<natoms;i++)
        frozen[i] = 0;
    for (int i=natoms;i<npad+natoms;i++)
        frozen[i] = 1;
    ...
    for (int i=0;i<nnmax;i++)
    {
        coords[i] = new double[1+(npad+natoms)*3];
        tangents[i] = new double[1+(npad+natoms)*3];
        grads[i] = new double[1+(npad+natoms)*3];
        perp_grads[i] = new double[1+(npad+natoms)*3];
    }

    printf("  -reading coordinates from xyz file \n");
    infile.open(xyzfile.c_str());
    fflush(stdout);
...
void GString::scan_r(int weig)
{
    ...
    grad1.init(infile0,natoms,anumbers,anames,runNum,runend,ncpu,3);
...
void GString::ic_reparam_g(double** dqa, double* dqmaga) 
{
    ...
    double* rpmovep = new double[nnmax];
    for (int i=0;i<nnmax;i++) rpmovep[i] = 0.;
    ...
    for (int i=0;i<ic_reparam_steps;i++)
    {
...
#if 0
        for (int n=n0+1;n<nnmax-1;n++)
            printf(" 0disp[%i]: %5.2f",n,rpmove[n]);
        printf("\n");
#endif

        MAXRE *= reparam_step_control(rpmovep,rpmove);
        if (MAXRE>0.25) MAXRE = 0.2;
        printf(" MAXRE: %1.3f",MAXRE);
        ...
        for (int n=n0+1;n<nnmax-1;n++)
            if (rpmove[n] > 0.) //tangent points inward, so don't move other direction
            {
                ...
                newic.dq0[newic.nicd0-1] = rpmove[n]; //CPMZ HERE
                newic.ic_to_xyz();
...
void GString::ic_reparam(double** dqa, double* dqmaga, int rtype) 
{
    ...
    double* rpmovep = new double[ictalloc];
    for (int i=0;i<ictalloc;i++) rpmovep[i] = 0.;
    ...
    for (i=0;i<ic_reparam_steps;i++)
    {
        ...
        if (rtype==0 && i==0)
        {
            if (Utils::isZero(nnmax-1-n0))
                std::cout << "ERROR: The number is zero on line " << __LINE__ << " of file " << __FILE__ << std::endl;
            if (Utils::isZero(TSnode-n0))
                std::cout << "ERROR: The number is zero on line " << __LINE__ << " of file " << __FILE__ << std::endl;
            if (Utils::isZero(nnmax-TSnode-1))
                std::cout << "ERROR: The number is zero on line " << __LINE__ << " of file " << __FILE__ << std::endl;
        ...
        if (rtype==1 && i==0)
        {
            ...
            if (Utils::isZero(totaledq))
                std::cout << "ERROR: The number is zero on line " << __LINE__ << " of file " << __FILE__ << std::endl;
            for (int n=n0+1;n<nnmax;n++)
                rpart[n] = 1.0 + edist[n] / totaledq;
            for (int n=n0+1;n<nnmax;n++)
            {
                if (Utils::isZero(rpart[n]))
                    std::cout << "ERROR: The number is zero on line " << __LINE__ << " of file " << __FILE__ << std::endl;
                rpart[n] = 1/rpart[n];
            }
            double norm = 0.;
        ...
        double disprms0 = 0.;
        for (int n=n0+1;n<nnmax-1;n++)
            disprms0 += rpmove[n]*rpmove[n];
        double gamma = disprms0/lastdispr; 
        if (gamma>0.5) gamma = 0.5; 
        printf(" gamma: %4.3f",gamma);
        for (int n=0;n<nnmax;n++)
            rpmove[n] += rpmovep[n]*gamma;
#endif
        MAXRE *= reparam_step_control(rpmovep,rpmove);
        if (MAXRE>0.25) MAXRE = 0.2;
        printf(" MAXRE: %1.3f",MAXRE);
        ...
        //    if (disprms<0.02) break;
        if (disprms<0.02 && disprms<MAXRE*2.) break;
...
#if 1
    printf(" spacings (end ic_reparam, steps: %i): ",i);
    for (int n=1;n<nnmax;n++)
        printf(" %1.2f",dqmaga[n]);
    printf("  disprms: %1.3f ",disprms);
    printf("\n");
...
double GString::reparam_step_control(double* rpmovep, double* rpmove)
{
    double scale = 1.;
    int slow = 0;
    int fast = 0;
    //    if (disprms0>lastdispr) slow = 1;
    for (int n=n0+1;n<nnmax-1;n++)
    {
        //printf(" rpmp/rpm: %1.2f %1.2f",rpmovep[n],rpmove[n]);
        if (Utils::isZero(rpmovep[n]))
            std::cout << "ERROR: The number is zero on line " << __LINE__ << " of file " << __FILE__ << std::endl;
        if (rpmove[n]/rpmovep[n]<-0.8 &&
                fabs(rpmove[n])>0.02)
            slow = 1;
        if (rpmove[n]/rpmovep[n]<0.7 && rpmove[n]/rpmovep[n]>0.0)
            fast = 1;
    }
    if (slow) scale *= 0.5;
    if (fast && !slow) scale *= 1.5;
    //printf(" scale: %1.3f",scale);
...
void GString::get_tangents_1(double** dqa, double* dqmaga, double** ictan)
{
    ...
    for (int n=1;n<nnmax-1;n++)
    {
        ...
        for (int i=0;i<natoms;i++)
            if (newic.xyzic[i])
            {
                printf("  %6.3f",ictan[n][size_icp+cxyzic++]);
                printf(" %6.3f",ictan[n][size_icp+cxyzic++]);
                printf(" %6.3f",ictan[n][size_icp+cxyzic++]);
            }
        printf("\n");
...
void GString::get_tangents_1g(double** dqa, double* dqmaga, double** ictan)
{
    ...
    {
        //printf("  get_tangents_1g nlist: %2i %2i \n",nlist[2*n+1],nlist[2*n+0]);
    ...
    for (int n=0;n<ncurrent;n++)
    {
        ...
        for (int i=0;i<natoms;i++)
            if (newic.xyzic[i])
            {
                printf("  %6.3f",ictan[nlist[2*n]][size_icp+cxyzic++]);
                printf(" %6.3f",ictan[nlist[2*n]][size_icp+cxyzic++]);
                printf(" %6.3f",ictan[nlist[2*n]][size_icp+cxyzic++]);
            }
        //    for (int i=size_ic;i<size_icp;i++)
        //      printf(" %1.3f",ictan[nlist[2*n]][i]);
        printf("\n");
    }
#endif
...
{
    printf(" IS THIS USED? \n");
    printf(" also: not implemented for xyzic \n");
    exit(1);
...
int GString::read_string(string stringfile, double** coordsn, double* energies)
{
    ...
    if (!infile){
        cout << "!!!!Error opening string file!!!!" << endl;
        exit(-1);
    }
...
void GString::growth_iters(int max_iter, double& totalgrad, double& gradrms, double endenergy, string strfileg, int& tscontinue, double gaddmax, int osteps, int oesteps, double** dqa, double* dqmaga, double** ictan)
{
    ...
    for (;oi<max_iter;oi++)
    {
        ...
        {
            pastts = past_ts();
            if (pastts || (addednode==0 && nn>2)) //checks for TS starting with n0-1 (was 0, previously n0)
            {
        ...
        int nnprint = nnR;
        if (!isSSM && !isFSM) nnprint = nnmax;
        print_string(nnprint,allcoords,strfileg+endg);
#if 1
