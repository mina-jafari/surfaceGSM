...
int ICoord::bmat_alloc() {
  ...
  for (int i=0;i<max_torsions+100;i++) torv0[i] = 0;
...
int ICoord::grad_init(string infilename, int ncpu, int run, int rune, int knnr_level) 
{
  ...
  grad1.init(infilename,natoms,anumbers,anames,run,rune,ncpu,knnr_level);
  g_inited = 1;
  //printf(" knnr_level: %i \n",knnr_level);
...
int ICoord::bmatp_create() {
 ...
 // printf(" in bmatp_create \n");
  ...
  for (int i=0;i<len;i++)
  {
    ...
    printf(" xyz components for IC %i: %5.3f %5.3f %5.3f \n",i,x[0],x[1],x[2]);
    x[0]=x[1]=x[2] = 0.;
  }
  delete [] x;
#endif
...
//      printf(" tordiff: %1.1f, effective tor_val: %1.1f ",tordiff,torsion_val(torsions[j][0],torsions[j][1],torsions[j][2],torsions[j][3])+torfix[j]);
 ...
 // printf(" before invert bbti \n"); fflush(stdout);
  ...
  double* bbteig = new double[len_d];
  Diagonalize(bbt,bbteig,len_d);
  for (int i=0;i<len_d;i++)
    printf(" %8.5f",bbteig[i]);
  printf("\n");
  delete [] bbteig;
  exit(1);
#endif
...
#if 1
  if (gradrms>gradrmsl*3.0 && energy>energyl)
  {
    if (DMAX>smag)
      DMAX = smag/1.5;
    else
...
#endif
  sprintf(sbuff," predE: %2.2f ",dEpre); printout += sbuff;
...
#if HESS_TANG
//if overlap is small, use Cn constraint
  if (maxol<HESS_TANG_TOL_TS && !isDavid) // || gradrms>OPTTHRESH*40.) 
  {
    delete [] tmph;
    delete [] eigen;
    delete [] gqe;
...
int ICoord::ic_to_xyz_opt() {
  ...
  for (int i=0;i<len_icp+nxyzic;i++)
    printf(" %1.1f",qprim[i]);
  printf("\n");
#endif
...
double ICoord::prima_force()
{
  ...
  //return 0.;
  printf(" prima_force not implemented for xyzic \n");
  exit(1);
